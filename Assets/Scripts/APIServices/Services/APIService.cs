...
namespace AHEFramework.Service.HTTP
{
    ...
    {
        public static void SendData(HTTPRequestMethod method, string url, bool haveAuth = true, Action<UnityWebRequest> callbackAction = null, List<KeyValuePair<string, string>> headers = default, string body = "")
        {
            ...
            if (haveAuth && !string.IsNullOrEmpty(JWTTokenService.AccessToken))
            {
                ...
                HTTPService.SendData(method, url, request =>
                {
                    ...
                    {
                        headersLst.Add(new KeyValuePair<string, string>("Authorization", JWTTokenService.AccessToken));
                        HTTPService.SendData(method, url, callbackAction, headersLst.ToArray(), body);
                    }
                    else callbackAction(request);

                }, headersLst.ToArray(), body);

            }
            else
                HTTPService.SendData(method, url, callbackAction, headers.ToArray(), body);
